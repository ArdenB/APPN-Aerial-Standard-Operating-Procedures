...
- [ ] **Photo** · *Explicit* — [Panel Setup](Protocols/FlightDesign/StandardFlight/Standard_Flight.md#panel-setup): GRYFN 4-panel ELM reflectance set (11 / 30 / 56 / 82 %) laid out in brightness order on a level folding table, oriented along the planned flight direction, in an open unshaded area.
- [ ] **Photo** · *Explicit* — [Validation Panel Setup](Protocols/FlightDesign/StandardFlight/Standard_Flight.md#validation-panel-setup): Validation panel table with the 2-panel validation set, the elevated GCP on the table, and the paired ground GCP placed in the adjacent flight line perpendicular to the elevated GCP.
