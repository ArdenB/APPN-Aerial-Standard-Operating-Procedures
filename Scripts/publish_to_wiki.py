...
def render_home(manifest: dict[str, Any], date: str) -> str:
    ...
    lines = [
        ...
        "",
        f"Welcome to the APPN Aerial SOP wiki. This wiki publishes the "
        f"**locked revision {manifest['revision']}** ({date}) of the standard "
        "operating procedures covering APPN UAV deployments, sensor "
        "configuration, data capture, quality assurance, and data management.",
        "",
        f"The working drafts (revisions in progress) live in the "
        f"[main repository]({repo_link}). Pages on this wiki are regenerated "
        "by `Scripts/publish_to_wiki.py` whenever a new revision is cut.",
        "",
    ]
    ordered = [c for c in cats] + [c for c in by_cat if c not in cats]
    for cat_id in ordered:
...
def write_navigation(manifest: dict[str, Any], wiki_root: Path, date: str,
    ...
    print(f"  nav:  Home.md, _Sidebar.md")
    if dry_run:
        return
